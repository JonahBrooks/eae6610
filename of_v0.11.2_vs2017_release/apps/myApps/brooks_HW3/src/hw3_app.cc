...
bool ParseGraph(const char* path, brooks_hw3::Graph& out_graph_to_fill) {
  char marker = 'c';
  ...
  int second_int = 0;
  int third_int = 0;
  std::vector<brooks_hw3::Edge> edges;
...
brooks_hw3::DynamicSteeringOutput LookInDirectionOfTravel(
    brooks_hw3::AiAgent agent) {
  ...
  return brooks_hw3::AiBehaviors::LookWhereYouAreGoing(
      agent.rigidbody_, brooks_hw3::Rigidbody2d(), max_rotation,
      align_slow_angle, align_target_angle, align_time_to_target);
}
}  // namespace
...
void Hw3App::setup() {
  ...
  for (int i = 0; i < grid_width_; i++) {
    for (int j = 0; j < grid_height_; j++) {
      grid_.push_back(ofVec2f(i * grid_square_world_width_ + grid_offset_.x,
                              j * grid_square_world_height_ + grid_offset_.y));

      // Make down pointing edge
      if (j < grid_height_ - 1) {
        edges_.push_back(Edge(i + (j * grid_width_),
                              i + (j * grid_width_) + grid_width_, 1));
      }
      ...
      if (i < grid_width_ - 1) {
        edges_.push_back(
            Edge(i + (j * grid_width_), (i + 1) + (j * grid_width_), 1));
      }
...
void Hw3App::draw() {
  ...
  ofSetColor(ofColor::black);
  ofDrawBitmapString("Log Heuristic Analysis",
                     heuristic_analysis_button_.position + ofVec3f(10, 20, 0));
}

//--------------------------------------------------------------
void Hw3App::keyPressed(int key) {}

//--------------------------------------------------------------
void Hw3App::keyReleased(int key) {}

//--------------------------------------------------------------
void Hw3App::mouseMoved(int x, int y) {}

//--------------------------------------------------------------
void Hw3App::mouseDragged(int x, int y, int button) {}

//--------------------------------------------------------------
void Hw3App::mousePressed(int x, int y, int button) {
...
//--------------------------------------------------------------
void Hw3App::mouseReleased(int x, int y, int button) {}

//--------------------------------------------------------------
void Hw3App::mouseEntered(int x, int y) {}

//--------------------------------------------------------------
void Hw3App::mouseExited(int x, int y) {}

//--------------------------------------------------------------
void Hw3App::windowResized(int w, int h) {}

//--------------------------------------------------------------
void Hw3App::gotMessage(ofMessage msg) {}

//--------------------------------------------------------------
void Hw3App::dragEvent(ofDragInfo dragInfo) {}

void Hw3App::RunHeuristicAnalysis() {
  ...
  for (size_t i = 0; i < number_of_random_walkthroughs; i++) {
    nodes_visited = 0;
    ofResetElapsedTimeCounter();
    std::vector<Edge> slc_path = AiPathfinding::Search(
        (int)ofRandom(0, slc_graph_.GetNumberOfNodes()),
        (int)ofRandom(0, slc_graph_.GetNumberOfNodes()), slc_graph_,
  ...
  std::cout << "SLC graph with A* GuessMinimumEdgeWeightHeuristic:"
            << std::endl;
  std::cout << "\tWalkthroughs: " << number_of_random_walkthroughs << std::endl;
  std::cout << "\tTotal Nodes Visited: "
            << total_nodes_visited_across_walkthroughs << std::endl;
  std::cout << "\tAverage Nodes Visited: "
            << (float)total_nodes_visited_across_walkthroughs /
                   (float)number_of_random_walkthroughs
            << std::endl;
  std::cout << "\tTotal Time Taken: " << total_time_across_walkthroughs
            << std::endl;
  std::cout << "\tAverage Time Taken: "
            << total_time_across_walkthroughs / number_of_random_walkthroughs
            << std::endl
            << std::endl;
  ...
  for (size_t i = 0; i < number_of_random_walkthroughs; i++) {
    ...
    std::vector<Edge> slc_path = AiPathfinding::Search(
        (int)ofRandom(0, slc_graph_.GetNumberOfNodes()),
        (int)ofRandom(0, slc_graph_.GetNumberOfNodes()), slc_graph_,
        nodes_visited, HeuristicType::kGuess1);
    total_time_across_walkthroughs += ofGetElapsedTimef();
    total_nodes_visited_across_walkthroughs += nodes_visited;
  }
  ...
  for (size_t i = 0; i < number_of_random_walkthroughs; i++) {
    ...
    std::vector<Edge> slc_path = AiPathfinding::Search(
        (int)ofRandom(0, slc_graph_.GetNumberOfNodes()),
        (int)ofRandom(0, slc_graph_.GetNumberOfNodes()), slc_graph_,
        nodes_visited, HeuristicType::kDijkstras);
    total_time_across_walkthroughs += ofGetElapsedTimef();
    total_nodes_visited_across_walkthroughs += nodes_visited;
  }
  ...
  for (size_t i = 0; i < number_of_random_walkthroughs; i++) {
    nodes_visited = 0;
    ofResetElapsedTimeCounter();
    std::vector<Edge> nyc_path = AiPathfinding::Search(
        (int)ofRandom(0, nyc_graph_.GetNumberOfNodes()),
        (int)ofRandom(0, nyc_graph_.GetNumberOfNodes()), nyc_graph_,
        nodes_visited, HeuristicType::kGuessMinimumEdgeWeight);
    total_time_across_walkthroughs += ofGetElapsedTimef();
    total_nodes_visited_across_walkthroughs += nodes_visited;
  }
  std::cout << "NYC graph with A* GuessMinimumEdgeWeightHeuristic:"
            << std::endl;
  std::cout << "\tWalkthroughs: " << number_of_random_walkthroughs << std::endl;
  std::cout << "\tTotal Nodes Visited: "
            << total_nodes_visited_across_walkthroughs << std::endl;
...
void Hw3App::MakeWallAndEditEdges(size_t x, size_t y, size_t width,
                                  size_t height) {
  ofVec2f wall_offset(grid_square_world_width_ / 4,
                      grid_square_world_height_ / 4);
  walls_.push_back(
      ofRectangle(GridToWorld(x, y) + wall_offset,
                  grid_square_world_width_ * width - wall_offset.x * 2,
                  grid_square_world_height_ * height - wall_offset.y * 2));
  ...
  for (size_t i = x; i < x + width; i++) {
    for (size_t j = y; j < y + height; j++) {
      ...
      if (j < grid_height_ - 1 && i != x) {
        to_remove = std::find(edges_.begin(), edges_.end(),
                              Edge(i + (j * grid_width_),
                                   i + (j * grid_width_) + grid_width_, 1));
        if (to_remove != edges_.end()) {
          edges_.erase(to_remove);
        }
        to_remove = std::find(edges_.begin(), edges_.end(),
                              Edge(i + (j * grid_width_) + grid_width_,
                                   i + (j * grid_width_), 1));
        if (to_remove != edges_.end()) {
          edges_.erase(to_remove);
        }
      ...
      if (i < grid_width_ - 1 && j != y) {
        ...
        to_remove = std::find(
            edges_.begin(), edges_.end(),
            Edge((i + 1) + (j * grid_width_), i + (j * grid_width_), 1));
        if (to_remove != edges_.end()) {
          edges_.erase(to_remove);
        }
      }
...
ofVec2f Hw3App::GridToWorld(size_t x, size_t y) {
  return ofVec2f(x * grid_square_world_width_ + grid_offset_.x,
                 y * grid_square_world_height_ + grid_offset_.y);
}

ofVec2f Hw3App::GridToWorld(size_t i) {
  size_t x = (i) % grid_width_;
  size_t y = (i) / grid_height_;
  return GridToWorld(x, y);
}

size_t Hw3App::WorldToGrid(ofVec2f location) {
  size_t x = (location.x - grid_offset_.x + grid_square_world_width_ / 2) /
             grid_square_world_width_;
  size_t y = (location.y - grid_offset_.y + grid_square_world_height_ / 2) /
             grid_square_world_height_;
